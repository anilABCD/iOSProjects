...
//    @Binding var path :[MyNavigation<String>]
    ...
    @State var isFristTimeSmokingUpdate = false;
    ...
    @State var items: [SelectableItem] = [
        ...
        SelectableItem(name: "Regularly"),
        SelectableItem(name: "Trying to Quit"),
    ...
    func submitSelections( authToken : String ) {
       ...
       request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        ...
        let body = [ "smoking" : selectedItem.name  ] as [String : String]
           ...
           if let responseMessage = try? JSONDecoder().decode(AuthResponse.self, from: data) {
               DispatchQueue.main.async {
                   ...
                   if self.status == "success" {
                       ...
                       if ( !isFristTimeSmokingUpdate && showNextButton == false ) {
                           presentationMode.wrappedValue.dismiss()
                       }
                   }
                   
                   self.tokenManger.updateSmoking(  smoking: selectedItem.name )
...
//
                ...
                if let index = items.firstIndex(where: { $0.name == tokenManger.smoking }) {
...
#if DEBUG
struct UpdateSmokingView_Previews: PreviewProvider {
//    @State static var path: [MyNavigation<String>] = [] // Define path as a static state variable
    static var previews: some View {
        UpdateSmokingView( ).environmentObject(TokenManager()).environmentObject(ThemeManager())
    }
}
#endif
