...
struct ProgressIndicatorExample: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 30) {
            ...
            Text("Progress IndicatorE xample").fontWeight(.bold).font(.largeTitle)
            
            Text("Task Progress: Progress View ").fontWeight(.bold)
            ...
            Text("Loading Indicator : Progress View ").fontWeight(.bold)
            
            ProgressView()
        }.frame(height: 500).border(.blue)
        .padding()
    }
}
